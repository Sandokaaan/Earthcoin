#!/usr/bin/env bash
# Copyright (c) 2016 The Earthcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   earthcoincore.org and earthcoin.org and compares them.
...
WORKINGDIR="/tmp/earthcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://earthcoincore.org"
HOST2="https://earthcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="earthcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'earthcoin-',
...
if [ $? -ne 0 ]; then
   echo "earthcoin.org failed to provide signature file, but earthcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "earthcoin.org and earthcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Earthcoin Core binary release signing key installed?"
   fi
