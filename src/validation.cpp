                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CAmount blockReward = nFees + GetEarthcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus());
...
{
    LOCK(cs_main);
    return (pindexPrev ? (pindexPrev->nHeight > params.nLegacyBlocksBefore) : false); 	
}
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Earthcoin: We probably want to disable this
    int nLockTimeFlags = 0;
    ...
    if (pindexPrev ? (pindexPrev->nHeight > consensusParams.nLegacyBlocksBefore) : false) {	    
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
