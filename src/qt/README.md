This directory contains the EarthcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Earthcoin UI.
...
ClientModel is used by the main application `earthcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/earthcoin/earthcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a earthcoin:... link from a browser).
...
* UI elements like EarthcoinAmountField, which inherit from QWidget.
* `earthcoinstrings.cpp`: automatically generated
* `earthcoinunits.(h/cpp)`: EAC / mEAC / etc handling
* `callback.h`
...
* don't change `local/earthcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "earthcoin-qt" under "Run", which is where you can also add command line arguments)
