...
const char * const EARTHCOIN_CONF_FILENAME = "earthcoin.conf";
const char * const EARTHCOIN_PID_FILENAME = "earthcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "earthcoind -foo=bar
...
#else
    const char* pszModule = "earthcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Earthcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Earthcoin
    // Mac: ~/Library/Application Support/Earthcoin
    // Unix: ~/.earthcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Earthcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Earthcoin";
#else
    // Unix
    return pathRet / ".earthcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EARTHCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", EARTHCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Earthcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Earthcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Earthcoin Core developers";
    }
