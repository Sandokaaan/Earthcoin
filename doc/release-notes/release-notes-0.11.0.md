Earthcoin Core version 0.11.0 is now available from:

  <https://earthcoin.org/bin/earthcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer EAC/kB fee to be
...
For example, add the following to `earthcoin.conf`:
...
related to the blockchain in the earthcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Earthcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Earthcoin Core will 
...
For an example script see [earthcoin-submittx](https://github.com/laanwj/earthcoin-submittx).
...
- #5334 `cf87536` libearthcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new earthcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only EarthcoinAmountField
- #5753 `0f44672` Add earthcoin logo to about screen
...
- #5809 `46bfbe7` Add earthcoin-cli man page
...
- #5849 `41259ca` contrib/init/earthcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Earthcoin-Qt.app to Earthcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for earthcoin-tx)
